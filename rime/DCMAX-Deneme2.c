...
static void updateEnegeryWeightMatrix(int i, float currentEnergyOf_i,float weight[][MAX], float distance[][MAX], int messageLength) {
	/*calculate  alfa_ij()*/
	float alfa_ij = 1 - (currentEnergyOf_i / 30.0);
	int j;
	for (j = 0; j < MAX; j++) {
		if (i != j) {
...
static void initializeWeightMatrix(float weight[][MAX], float distance[][MAX],
	...
	int i;
	for (i = 0; i < MAX; i++) {
		updateEnegeryWeightMatrix(i, 30.0, weight, distance, messageLength);
	}
	printf("FINISH-3 \n ");
}
...
/*Send DataPackage*/
static int8_t forwardPathToPackageBuffer(int destinationNode,uint16_t currentHop){
	int preced[MAX],j;
...
//    updateEnegeryWeightMatrix(1,10,weight,distance,1);
//    updateEnegeryWeightMatrix(2,10,weight,distance,1);
//    updateEnegeryWeightMatrix(5,10,weight,distance,1);
			/*clear packetBuf to ensure that all information about forwarding was deleted.*/
//			if(roundCount==10 && (bateryLevel-1)>-1){
//			packetbuf_clear();
